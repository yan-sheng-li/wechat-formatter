...
TypeZen 是一款专为微信公众号设计的「Markdown 转微信排版」辅助工具。用户编写 Markdown 文本后，可以使用 AI 一键优化排版结构，再套用 96 套不同风格的精美模板，可直接复制粘贴到微信公众平台后台。
...
- **96 套模板**分为 8 大类：
  ...
  - `chinese-classic` - 国风（12套）：宣纸底色、印章式标记、竖线章节符
  - `tech` - 科技风（12套）：尖角、极客终端风格
  - `data-report` - 数据资讯风（12套）：刊头式标题、数据表格、圆形项目符
...
- 已完成模板扩展到 96 套，当前分类为新粗野、极简、商务、文艺、国风、科技、数据资讯、节庆 8 类
